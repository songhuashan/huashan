...
class CommentWidget extends Widget {
	...
	public function render($data) {
		...
		if ($var ['showlist'] == 1) { // 默认只取出前10条
			...
			foreach($var ['list']['data'] as $key =>$val)
            {
                $comment['table'] = 'group_topic';
                $comment['to_comment_id'] = $val['comment_id'];
                $var ['list']['data'][$key]['commentcounts'] = M('comment') -> where($comment) -> count();

           if($_GET['mod']  == 'Topic') {
               $topicmap['table'] = 'zy_topic';
               $topicmap['to_comment_id'] = $val['comment_id'];
               $var ['list']['data'][$key]['commentcounts'] = M('comment')->where($topicmap)->count();
           }
            }
		} // 渲染模版
