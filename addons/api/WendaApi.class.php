...
class WendaApi extends Api{
    ...
    */
	...
	public function getAnswer(){
		$map['d.uid']    = $this->mid;
		$map['d.is_del'] = 0;
		$list = M("zy_wenda_comment d")->join("`".C('DB_PREFIX')."zy_wenda` w ON w.id = d.wid")->field('w.id,w.uid,wd_description,w.wd_comment_count,w.wd_browse_count,w.ctime')->where($map)->limit($this->_limit())->order('ctime DESC')->select();
		foreach($list as &$val){
			$val['uname']    = getUserName($val['uid']);
			$val['userface'] = getUserFace($val['uid'],'m');
			$val['ctime']    = date('Y-m-d',$val['ctime']);
	...
	public function postWenda(){
        ...
        $content = strip_tags(filter_keyword($this->data['content']), '<a><br><span><b><i><strong><img>');
		...
		if($res){
            //操作积分
            model('Credit')->getCreditInfo($this->mid,34);
			$this->exitJson($res);
		}
	...
	public function doWendaComment(){
        ...
        $cont = strip_tags(filter_keyword($this->data['content']), '<a><br><span><b><i><strong><img>');//获取评论内容
        if( !$wid || !$cont) $this->exitJson( array() ,10033,"对不起，操作失败");
        $data=array(
            'uid'=>$this->mid,
            'wid'=>$wid,
        ...
        if($res){//评论成功
			//操作积分
			model('Credit')->getCreditInfo($this->mid,35);
            //设置问答评论数量+1
	...
	public function detail(){
		...
		if($wdata){
            ...
            $wdata['follow_state'] = model('Follow')->getFollowState($this->mid , $wdata['uid']);
			$this->exitJson($wdata);
		}else{
