                ...
                //生成节点
                    s = o(
                        [
                            ...
                            function() {
                                return l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + '</i>' : '<i style="padding-left: 18px;"></i>';
                            }(),
                            function() {

                                //如果用户选择的风格是checkbox则生成checkbox
                                var eleStr = i.check&&i.check=="checkbox" ? 'padding-left<input type="checkbox" name="'+i.checkboxName+'" '+((n.checked&&n.checked==true) ? 'checked="checked"' : "")+(n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid +'"' + 'id="' + n.id + '"' +(i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "" ) :  '';
                                if(eleStr.length > 0){
                                    //给元素添加data-xxx属性
                                    if(n.data && Object.prototype.toString.call(n.data) == "[object Object]"){
