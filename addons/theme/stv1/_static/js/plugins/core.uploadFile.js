			...
			　var _this = this;
			 core.loadFile(THEME_URL+'/js/jquery.form.js',function(){
				...
				if($(_this.resultDiv).find('.loading').size() < 1 ){
					$(_this.resultDiv).find('ul').html('<li class="loading"><div class="loads"><img src="'+THEME_URL+'/image/load.gif" style="width:auto;height:auto"></div><p class="tips upload_tips" style="padding:5px"><a href="javascript:core.uploadFile.stopupload()">取消上传</a></p></li>');//'+L('PUBLIC_REMOVE_UPLOAD')+'	
				}
...
/*			var hasUpload = false;
			...
			if('' !=func){
				...
				if(this.type=='image'){
    				...
    				var html = '<li><i class="ico-'+data.extension+'-small 123123"></i><a class="ico-close right" href="javascript:void(0)" onclick="core.uploadFile.removeAttachId(this,\''+this.type+'\','+data.attach_id+')"></a>'
