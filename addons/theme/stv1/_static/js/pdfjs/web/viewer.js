...
var DEFAULT_URL = 'EdulineÓÃ»§Ö¸ÄÏ.pdf';
  ...
  PDFPresentationMode.prototype = {
    ...
    request: function PDFPresentationMode_request() {
      ...
      if (this.container.requestFullscreen) {
		...
		var data_file = document.location.search.replace('?file=','');
		var file_url = '/addons/theme/stv1/_static/js/pdfjs/web/viewer.html?fr=full&file='+data_file;
...
function webViewerInitialized() {
  ...
  //var file = queryString.replace('file=','');
...
/*
  ...
  */
  ...
  var docfullscreenElems = getElmsByClsName('docfullscreen');
  for(var i =0;i < docfullscreenElems.length;i++){
	  var clickElement = docfullscreenElems[i];
	  clickElement.addEventListener('click',function(element){
		   var element = document.getElementById('showdocbody');
		   docfullscreen(element);
	  });
...
// doc fullscreen
function docfullscreen(element){
	// 是否全屏
	var fullscreenElement = document.fullscreenEnabled || document.mozFullscreenElement || document.webkitFullscreenElement || document.msFullscreenElement;
	if(fullscreenElement){
		exitFullscreen();
		return true;
	}
	// 如果包含参数fr=full,退回上一页
	if(GetQueryString('fr')== 'full'){
		window.history.go(-1);
		return true;
	}
