...
$(".Toolbar_inbox").next("div").siblings("a").each(function(item,value){
    if(item == 0 || item == 3){$(this).css({"float:left","margin-left":"2%"})}
    if(item == 1 || item == 4){$(this).css({"float:left"})}
    if(item == 2 || item == 5){$(this).css({"float:left"})}
});
/**
 * 处理ajax返回数据之后的刷新操作
 */
