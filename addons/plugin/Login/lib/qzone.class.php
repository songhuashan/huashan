...
class qzone{
	...
	public function userInfo(){
		...
		$get_unionid = "https://graph.qq.com/oauth2.0/me?access_token={$_SESSION['qzone']['access_token']['oauth_token']}&unionid=1";
        $chd = curl_init();
        curl_setopt($chd, CURLOPT_URL,$get_unionid);
        curl_setopt($chd, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($chd, CURLOPT_SSL_VERIFYHOST, false);
        curl_setopt($chd, CURLOPT_RETURNTRANSFER, 1);
        $unionid = curl_exec($chd);
        curl_close($chd);
        $unionid_info = json_decode("{".explode('}',explode('{',$unionid)[1])[0]."}");

		$user['id']          = $unionid_info->unionid;
