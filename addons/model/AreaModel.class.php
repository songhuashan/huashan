...
class AreaModel extends Model {
	 ...
	 */
	public function getAreaName($area_ids,$delimiter = ' ')
	{
		$area_name = '';
		if(!is_array($area_ids)){
			$area_ids = array_filter(explode(',',$area_ids));
		}
		foreach ($area_ids as $area_id) {
			if($area_id){
				 $title = $this->where(['area_id'=>$area_id])->getField('title') ?:'';
				 $title && $area_name .= $title.$delimiter;
			}
		}
		return rtrim($area_name,$delimiter);
	}
}