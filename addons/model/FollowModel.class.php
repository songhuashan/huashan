...
class FollowModel extends Model {
	...
	protected $fields = array(0=>'follow_id',1=>'uid',2=>'fid',3=>'tid',4=>'remark',5=>'ctime','_autoinc'=>true,'_pk'=>'follow_id');
	 ...
	 * @param integer $fid 被关注的讲师ID
	 * @return boolean 是否关注成功
	 */
	public function doFollow($uid, $fid,$tid = 0) {
		if ( intval( $uid ) <= 0 || $fid <= 0 || $tid < 0){
        ...
        if($tid){
    		$tid = D('ZyTeacher')->where("uid=$fid")->getField('id');
            if(!$tid){
            	$this->error = L('PUBLIC_FOLLOWING_PEOPLE_NOEXIST');			//被关注的讲师不存在
    			return false;
            }
        }
		...
		// 获取双方的关注关系
		$follow_state = $this->getFollowState($uid, $fid,$tid);
 		// 未关注状态
...
/*			model('Notify')->send($fid, 'user_follow', '', $uid);
     ...
     * @return boolean 是否关注成功
     */
    public function doFollowMhm($uid, $fid) {
...
// ***************************************************ts2.XX  应用移动新增函数
	 ...
	 * @param string $type 关键字，following,follower
	...
	public function searchFollows ($key, $type, $limit = 20, $uid, $gid) {
		...
		if ($type === 'following') {
			$map['a.`uid`'] = $uid;
			$map['b.`uname`'] = array('LIKE', '%'.$key.'%');
			$gid = empty($gid) ? 0 : intval($gid);
			switch ($gid) {
				case 0:
					$list = D()->table('`'.C('DB_PREFIX').'user_follow` AS a LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`fid` = b.`uid`')
							   ->field('a.`fid`')
							   ->where($map)
							   ->findPage($limit);
					break;
				case -1:
					$map['c.`uid`'] = array('EXP', 'IS NOT NULL');
					$list = D()->table('`'.C('DB_PREFIX').'user_follow` AS a LEFT JOIN `'.C('DB_PREFIX').'user_follow` AS c ON a.`uid` = c.`fid` AND c.`uid` = a.`fid` LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`fid` = b.`uid`')
				   			   ->field('a.fid')
				   			   ->where($map)
				   			   ->order('a.follow_id DESC')
				   			   ->findPage($limit);
				...
				case -2:
					$uid = intval($uid);
					$map['c.`fid`'] = array('EXP', 'IS NULL');
					$list = $this->table('`'.C('DB_PREFIX').'user_follow` AS a LEFT JOIN `'.C('DB_PREFIX').'user_follow_group_link` AS c ON a.`fid` = c.`fid` LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`fid` = b.`uid`')
								 ->field('a.fid')
								 ->where($map)
								 ->order('a.follow_id DESC')
								 ->findPage($limit);
				...
				default:
					...
					$list = $this->table('`'.C('DB_PREFIX').'user_follow` AS a LEFT JOIN `'.C('DB_PREFIX').'user_follow_group_link` AS c ON a.follow_id=c.follow_id AND a.uid=c.uid LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`fid` = b.`uid`')
								 ->field('a.fid')
				    			 ->where($map)
				    			 ->order('a.follow_id DESC')
				    			 ->findPage($limit);
			...
			$map['a.`fid`'] = $uid;
			$map['b.`uname`'] = array('LIKE', '%'.$key.'%');
			$list = D()->table('`'.C('DB_PREFIX').'user_follow` AS a LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`uid` = b.`uid`')
					   ->field('a.`uid` AS `fid`')
					   ->where($map)
					   ->findPage($limit);
		}
