...
class PassportModel {
	...
	public function adminLogout() {
		unset($_SESSION['adminLogin'],$_SESSION['school_id']);
	...
	public function logoutforce() {
		echo "<script>alert('账号已被强制退出');</script>";
		
		//$this->mzSuccess = "账号已在其他地点登陆";
		
		unset($_SESSION['mid'],$_SESSION['SITE_KEY'],$_SESSION['is_teacher'],$_SESSION['loginkey']); // 注销session
	...
	public function isLogged() {
		...
		if(intval($_SESSION['mid']) > 0 && $_SESSION['SITE_KEY']==getSiteKey()) {
		   //return true;
		//验证用户key是否一样	
		  $userinfo=M('user')->find($_SESSION['mid']);
		...
		//判定用户是否已经被禁用
          ...
          if($_SESSION['mid']!=1){			
           if($userinfo['loginstr']==$_SESSION['loginkey']){
				return true;
			}else{
				$this->logoutforce();
			}  
	 ...
	 */
	public function getLocalUser($login, $password,$is_service = false) {
       ...
       //echo  model('User')->getLastSql();
		...
		if($is_service !== true){
            $password = md5(md5($password).$user['login_salt']);
        }
		...
		if($password && ($password != $user['password'])) {
			...
			$credit = M()->query("UPDATE `el_user` SET loginstr='1' WHERE uid='{$uid}'");
	 ...
	 * @param booelan $is_service 是否为服务器登录,如果为服务器登录,此处的密码为加密的密码
     * @return boolean 是否登录成功
     */
    public function loginLocal($login, $password = null, $is_remember_me = false, $getinfo = false,$is_service = false) {
        $res = false;
        if(UC_SYNC){
        ...
        $user = $this->getLocalUser($login, $password,$is_service);
        // APP登录接口改
        if ($getinfo && $user){
            unset($user['password']);
	...
	private function _recordLogin($uid, $is_remember_me = true) {
		...
		// 记住活跃时间
		cookie('TSV3_ACTIVE_TIME',time() + 60 * 60 * 24 * 30);
		cookie('login_error_time', null);
        $loginstr=md5(round(10000,99999).time());
		// 更新登录时间
		model('User')->setField('last_login_time', $_SERVER['REQUEST_TIME'], 'uid='.$uid);
		M()->query("UPDATE `el_user` SET loginstr='{$loginstr}' WHERE uid='{$uid}'");
		// 记录登录日志，首次登录判断
