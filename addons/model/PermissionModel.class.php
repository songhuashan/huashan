...
class PermissionModel {
	...
	public function check($action) {
		...
		if(model('UserGroup')->isAdmin($_SESSION['mid'])){
			return true;
		}else{
		...
		if(empty(self::$permission[$uid])) {
			...
			if(!$permission) {
				...
				foreach($userGroup as $k => $v) {
					...
					foreach($_p as $rule_id){
						// 根据权限节点ID获取权限规则名称
						$permission[] = M('permission_node')->where('id='.$rule_id)->getField('rule') ;
					}
				}
	...
	/**
	 * 获取指定用户组的权限
	 * @param string $key 用户组ID或者特殊应用下面的appname_app groupname
	 * @return array 指定用户组的权限信息
	 */
	public function getGroupPermission($key,$group_info) {
		static $permissionCache;
		if(!$permissionCache[$key]) {
			$cacheRule = model('Cache')->get('perm_'.$key);
			if(!$cacheRule) {
				$group_info = $group_info ?: model('UserGroup')->getUserGroup($key);
				//$cacheRule = model('Xdata')->get('permission:'.$key);
				$cacheRule = array_filter(explode(',',$group_info['rule_list']));
