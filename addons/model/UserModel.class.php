...
class UserModel extends Model {
	...
	public function addUser($user) {
		...
		$map2 ['phone'] = t ( $user ['phone'] );
		
		//$uname = $this->where ( $map1 )->count ();
		$phone = $this->where ( $map2 )->count ();
	...
	public function getUserFollowers(){
        ...
        $map['is_active'] = 1;
        $map['is_audit'] = 1;
        $uids = model('User')->where($map)->field('uid')->findALL();
		foreach ($uids as $k => $v) {
			$group_count = D('user_group_link')->where(array('uid'=>$v['uid']))->count();
            $user_group  = D('user_group_link')->where(array('uid'=>$v['uid'],'user_group_id'=>['eq',2]))->getField('id');
            $uids[$k]['follow'] = model('Follow')->where("`fid`=".$v['uid'])->count();
            if($group_count != 1 || ($group_count == 1 && !$user_group ) || $uids[$k]['follow'] < 1){
                unset($uids[$k]);
            }else{
