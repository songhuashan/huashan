...
class LiveCategoryModel extends Model {
    ...
    protected $tableName = 'zy_live_cetegory';
    ...
    public function getCategoryName($id, $isTop = false){
        if(null === self::$categoryNames){
            $field = 'zy_live_cetegory_id as id,pid,title';
    ...
    public function getParentId($id){
        return intval($this->where(array('zy_live_cetegory_id'=>$id))->getField('pid'));
    }
    ...
    public function getChildCategory($pid){
        return $this->where(array('pid'=>$pid))
            ->order('sort asc,zy_live_cetegory_id')->select();
    }
	...
	public function getCategoryList($pid = -1) {
		$map = array();
		$pid != -1 && $map['pid'] = $pid;
		$data = $this->where($map)->order('`zy_live_cetegory_id` ASC')->findAll();
		return $data;
	}
		...
		foreach($list as $k1 => $p) {
			if($p['pid'] == 0) {
				...
				foreach($list as $k2 => $c) {
					if($c['pid'] == $p['zy_live_cetegory_id']) {
						$city[] = array($c['zy_live_cetegory_id'] => $c['title']);
						unset($list[$k2]);
					}
				}
				$output['provinces'][] = array(
					'id' => $p['zy_live_cetegory_id'],
					'name' => $p['title'],
					'citys' => $city,
				);
				unset($list[$k1], $city);
...
//		}
    ...
    /**
     * 查询某个一级菜单下面的子级及其孙子级
     * 仅支持zy_live_cetegory通用表
     * 仅限三级菜单 三级以上自己写 mdjj
     * @param $zy_live_cetegory_id
     * @return $data
     */
    public function _MakeAllChildTree($zy_live_cetegory_id) {
        $zy_currency_idss = '';
        $zy_live_cetegory_id_arr = M('zy_live_cetegory')->where(array('pid'=>$zy_live_cetegory_id))->field('zy_live_cetegory_id')->select();
        $zy_live_cetegory_ids = trim(implode(',', getSubByKey($zy_live_cetegory_id_arr , 'zy_live_cetegory_id')),',');
        $zy_currency_idss .= implode(',',getSubByKey($zy_live_cetegory_id_arr , 'zy_live_cetegory_id')).',';
        $maps['zy_live_cetegory_id'] = array('in',$zy_live_cetegory_ids);
        $result = M('zy_live_cetegory')->where($maps)->field('zy_live_cetegory_id')->select();
        foreach ($result as $key => $val){
            $zy_live_cetegory_child_id_arr = M('zy_live_cetegory')->where(array('pid'=>$val['zy_live_cetegory_id']))->field('zy_live_cetegory_id')->select();
            $zy_live_cetegory_child_ids = trim(implode(',', getSubByKey($zy_live_cetegory_child_id_arr , 'zy_live_cetegory_id')),',');

            $child_map['zy_live_cetegory_id'] = array('in',$zy_live_cetegory_child_ids);
            $child_result[] = M('zy_live_cetegory')->where($child_map)->field('zy_live_cetegory_id')->select();
        }
        foreach ($child_result as $k => $v){
            $zy_currency_idss .= implode(',', getSubByKey($v , 'zy_live_cetegory_id')).',';
        }
        $all_map['zy_live_cetegory_id'] = array('in',trim($zy_currency_idss,','));
        $data = M('zy_live_cetegory')->where($all_map)->select();
	...
	private function _MakeTree($pid, $level = '0', $type = '1') {
		...
		if($result) {
			foreach($result as $key => $value) {
				$id = $value['zy_live_cetegory_id'];
				$list[$id]['id'] = $value['zy_live_cetegory_id'];
				$list[$id]['pid'] = $value['pid'];
				$list[$id]['title'] = $value['title'];
				$list[$id]['level'] = $level;
				$list[$id]['child'] = $this->_MakeTree($value['zy_live_cetegory_id'], $level + 1,$type);
			}
		}
...
//		$myIds = $myIds?implode(',',$myIds):0;
	...
	private function _getPids($_ids,&$myIds=array()){
		$ids  = array();
		$pids = $this->where(array('pid'=>array('in',(string)$_ids)))->field('zy_live_cetegory_id')->select();
		
		foreach($pids as $value){
			$ids[] = $value['zy_live_cetegory_id'];
			$myIds[] = $value['zy_live_cetegory_id'];
		}
