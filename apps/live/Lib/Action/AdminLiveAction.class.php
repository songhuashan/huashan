...
class AdminLiveAction extends AdministratorAction
{
    ...
    public function addLive()
    {
        if (isset($_POST)) {
            ...
            if (empty($_POST['cover_ids'])) {
                $this->error("请上传直播课堂封面");
            }
            //if(empty($_POST['video_binfo'])){$this->error("直播课堂简介信息不能为空");}
            if (empty($_POST['video_intro'])) {
                $this->error("直播课堂简介信息不能为空");
            }
            if ($_POST['v_price'] == '') {
                $this->error("市场价格不能为空");
            }
            if (!is_numeric($_POST['v_price'])) {
                $this->error('市场价格必须为数字');
            }
            if ($_POST['t_price'] == '') {
                $this->error("销售价格不能为空");
            }
            if (!is_numeric($_POST['t_price'])) {
                $this->error('销售必须为数字');
            }
            if (empty($_POST['listingtime'])) {
                $this->error("上架时间不能为空");
            }
            if (empty($_POST['uctime'])) {
                $this->error("下架时间不能为空");
            }
            if (strtotime($_POST['uctime']) < strtotime($_POST['listingtime'])) {
                $this->error("下架时间不能小于上架时间");
            }
            if (empty($_POST['maxmannums'])) {
                $this->error("最大并发量不能为空");
            }
            if (!is_numeric($_POST['maxmannums']) || preg_match("/^-[0-9]+(\.[0-9]+)?$/", $_POST['maxmannums'])) {
                $this->error('最大并发量格式错误');
            }
...
//        $this->opt['webJoin']    = ['0'=>'不限','1'=>'否','2'=>'是'];
    ...
    public function addZshdLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if ($status == 0) {
                ...
                if ($clientJoin == 'false' && $webJoin == 'false') {
                    $this->error('Web端学生加入或客户端开启学生加入必须开启其一');
                }
                if (empty($_POST['teacherToken'])) {
                    $this->error('老师口令不能为空');
                }
                if (!is_numeric($_POST['teacherToken'])) {
                    $this->error('老师口令必须为数字');
                }
                if (strlen($_POST['teacherToken']) < 6 || strlen($_POST['teacherToken']) > 15) {
                    $this->error('老师口令只能为6-15位数字');
                }
                if (empty($_POST['assistantToken'])) {
                    $this->error('助教口令不能为空');
                }
                if (!is_numeric($_POST['assistantToken'])) {
                    $this->error('助教口令必须为数字');
                }
                if (strlen($_POST['assistantToken']) < 6 || strlen($_POST['assistantToken']) > 15) {
                    $this->error('助教口令只能为6-15位数字');
                }
                if (empty($_POST['studentToken'])) {
                    $this->error('学生WEB端口令不能为空');
                }
                if (!is_numeric($_POST['studentToken'])) {
                    $this->error('学生WEB端口令必须为数字');
                }
                if (strlen($_POST['studentToken']) < 6 || strlen($_POST['studentToken']) > 15) {
    ...
    public function editZshdLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if ($status == 0) {
                ...
                if (!is_numeric($_POST['maxAttendees'])) {
    ...
    public function addCcLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if (empty($_POST['studentClientToken'])) {
                $this->error('学生口令不能为空');
            }
            if (!is_numeric($_POST['studentClientToken'])) {
                $this->error('学生口令必须为数字');
            }
            if (strlen($_POST['studentClientToken']) < 6 || strlen($_POST['studentClientToken']) > 15) {
...
//            $this->onsubmit = 'admin.checkAddCc(this)';
    ...
    public function addWhLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if (strlen(t($_POST['subject'])) > 50) {
                $this->error('直播课时名称不能超过50个字符/25个汉字');
            }
            if (empty($startDate)) {
                $this->error('开始时间不能为空');
            }
...
//
                ...
                //查询服务器
                if ($live_info_res->code != 200 || $teacher_join_url->code != 200) {
                    $this->error('服务器查询失败');
                }
...
//            $this->onload[] = 'admin.checkLoadCC()';
    ...
    public function addCcXbkLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if ($res['result'] == 'OK') {
                ...
                if ($live_info_res['result'] != 'OK') {
                    $this->error('服务器查询失败');
                }// || $live_url_info_res['result'] != 'OK'
    ...
    public function editCcXbkLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if (intval($_POST['maxAttendees']) > $this->cc_config['xbk_max_users']) {
                $this->error('最大支持人数不能超过开通人数上限');
            }
    ...
    public function addEeoXbkLiveRoom()
    {
        ...
        if (isset($_POST)) {
            ...
            if ($live_course_res->error_info->errno == 1 || $live_course_res->error_info->errno == 135) {
                ...
                if ($live_course_info_res->error_info->errno != 1) {
                    $this->error('服务器查询失败');
                }
...
//            $_POST['mhm_title'] && $map['mhm_id']  = model('School')->where(array('title'=>['like','%' . t($_POST['mhm_title']) . '%']))->getField('id');
        ...
        foreach ($liveInfo['data'] as $key => $val) {
            ...
            if ($val['clientJoin'] == 0) {
                $liveInfo['data'][$key]['clientJoin'] = '<span style="color: red;">关闭</span>';
            } else {
                $liveInfo['data'][$key]['clientJoin'] = '开启';
            }
            ...
            if ($val['is_activity'] == 3) {
                ...
                $liveInfo['data'][$key]['DOACTION'] .= '<a class="withdraw" '. 'attr-id="' . $val['id'] . '"'. 'attr-data="' . msubstr($val['video_title'], 0, 20) .'" href="javascript:void(0)">| 撤回赠送</a> ';
            }
        }
        return $liveInfo;
    }
    ...
    {
        $id = $_POST['id'];
        $group_course = M('groupGiving')->field("el_user_group.user_group_name,el_group_giving.kid,el_group_giving.gid")->join("RIGHT JOIN el_user_group  on el_user_group.user_group_id = el_group_giving.gid")->where("kid=" . $id)->select();
        

        if ($group_course) {
            $this->ajaxReturn($group_course, '', 1);
        } else {
            $this->ajaxReturn(null, '查不到数据', 2);
        }
    }
     ...
     */
     ...
     {
        $id = $_POST['id'];
        $res = M('groupGiving')->where("gid=".$id)->delete();
        if($res){
            $this->ajaxReturn(null, '撤回成功' , 1);
        }else{
            $this->ajaxReturn(null, '撤回失败', 2);
        }
