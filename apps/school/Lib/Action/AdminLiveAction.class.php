...
class AdminLiveAction extends AdministratorAction
{
    ...
    public function index(){
        ...
        $this->pageButton[] = array('title' => "禁用", 'onclick' => "admin.livedoaction('','ColseLive',9)");
    ...
    public function closeLive(){
        ...
        $this->pageButton[] = array('title' => "搜索", 'onclick' => "admin.fold('search_form')");
        $this->pageButton[] = array('title' => "彻底删除", 'onclick' => "admin.livedoaction('','ColseLive',9,1)");
...
//            }
            ...
            if(empty($_POST['video_intro'])){$this->error("直播课简介不能为空");}
    ...
    public function editLive(){
        if( isset($_POST) ) {
            ...
            $this->pageKeyList   = array('id','video_title','live_cate','school','teacher_id','cover','video_binfo','video_intro','vip_level','video_score','maxmannums','v_price','t_price','listingtime','uctime','is_charge','is_mount');
            ...
            //获取机构,讲师信息
            $live_info['school'] = model('School')->getSchooldStrByMap(['id'=>$live_info['mhm_id']],'title');
            $map = ['is_del'=>0,'mhm_id'=>$live_info['mhm_id']];
            $all_speaker = M('zy_teacher')->where($map)->getField('id,name');
            $this->opt['teacher_id'] = $all_speaker ? array('0'=>'请选择')+$all_speaker : array('0'=>'请选择');
...
//        $this->opt['webJoin']    = ['0'=>'不限','1'=>'否','2'=>'是'];
    ...
    public function eeoXbkLiveRoom(){
        ...
        $this->searchPostUrl = U('school/AdminLive/eeoXbkLiveRoom',['id'=>$live_id]);
    ...
    */
    ...
    public function addZshdLiveRoom(){
        ...
        if(isset($_POST)){
            ...
            $liveInfo = model('Live')->findLiveInfo(array('id'=>$live_id),'id,video_title,live_type,maxmannums,uctime,listingtime,is_activity');
    ...
    public function addEeoXbkLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            if($live_course_res->error_info->errno == 1 || $live_course_res->error_info->errno == 135){
                ...
                if(!$result){$this->error('创建失败!');}
                $this->assign( 'jumpUrl', U('school/AdminLive/eeoXbkLiveRoom',array('id'=>$live_id)) );
                $this->success('创建成功');
            }else{
            ...
            $this->savePostUrl = U('school/AdminLive/addEeoXbkLiveRoom',['id'=>$live_id]);
            $this->displayConfig(['maxAttendees'=>$liveInfo['maxmannums']]);
        }
    }
    ...
    public function editEeoXbkLiveRoom(){
        ...
        if( isset($_POST) ) {
            ...
            if($live_course_res->error_info->errno == 1 || $live_course_res->error_info->errno == 135){
                ...
                if(!$result){$this->error('编辑失败');}
                $this->assign( 'jumpUrl', U('school/AdminLive/eeoXbkLiveRoom',array('id'=>intval($_GET['live_id']))) );
                $this->success('编辑成功');
            }else{
                $this->error($live_course_res->error_info->error);
            }
            ...
            $this->savePostUrl = U('school/AdminLive/editEeoXbkLiveRoom',['id'=>$_GET['id'],'live_id'=>$live_id]);
            $this->displayConfig($live_room_data);
        }
    }
...
//            $liveInfo['data'][$key]['mhm_title'] = model('School')->getSchooldStrByMap($s_map,'title');
            ...
            }else if($val['is_activity'] == 3){
                $liveInfo['data'][$key]['is_activity'] = "<p style='color: green;'>提交平台审核</p>";
            }
            ...
            if( $val['is_activity']== 2){
                $liveInfo['data'][$key]['DOACTION'] .= ' | <a href="javascript:void();" onclick="admin.activeVideo(' . $val['id'] . ',\'AdminVideo\',true,0)">提交平台审核</a>';
                $liveInfo['data'][$key]['DOACTION'] .= '<a href="javascript:void();" onclick="admin.activeVideo(' . $val['id'] . ',\'AdminVideo\',false,0)">驳回</a>';
            }else if($val['is_activity']== 3){
                $liveInfo['data'][$key]['DOACTION'] .= '<span style="color:grey">等待平台审核</span>';
            }else{
                if($val['live_type'] == 1){
                    ...
                    $liveInfo['data'][$key]['DOACTION'] .= '<a href="'.U('school/AdminLive/addEeoXbkLiveRoom',array('id'=>$val['id'])).'">新建直播课时</a> | ';
                    $liveInfo['data'][$key]['DOACTION'] .= '<a href="'.U('school/AdminLive/eeoXbkLiveRoom',array('id'=>$val['id'])).'" title="查看此直播课堂下直播课时">直播课时</a> | ';
                }else if($val['live_type'] == 8){
...
//            $liveInfo['data'][$key]['DOACTION'] = '<a title="查看直播课堂详细信息" href="' . U('school/Admin/checkLive', array('tabHash' => 'checkLive', 'id' => $val["id"])) . '">查看</a> | ';
    ...
    private function _getCcLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_del'] == 0) {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'ColseLiveRoom\''.',4)">禁用</a> | ';
            } else {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'OpenLiveRoom\''.',4)">启用</a> | ';
            }
            ...
            if ($val['is_del'] == 1){
                $val['DOACTION'] .= ' | <a title="此操作会彻底删除数据" href="javascript:void(0)"  onclick="admin.livedoaction('.$val['id'].',\'DelLiveRoom\''.',4)">彻底关闭</a>  ';
            }
...
//            $val['webJoin']         = $val['webJoin'] ? '<p style="color: green;">开启</p>' : '<p style="color: red;">关闭</p>';
            ...
            if ($val['is_del'] == 0) {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'ColseLiveRoom\''.',5)">禁用</a> | ';
            } else {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'OpenLiveRoom\''.',5)">启用</a> | ';
            }
            ...
            if ($val['is_del'] == 1){
                $val['DOACTION'] .= ' | <a title="此操作会彻底删除数据" href="javascript:void(0)"  onclick="admin.livedoaction('.$val['id'].',\'DelLiveRoom\''.',5)">彻底关闭</a>  ';
            }
    ...
    private function _getCcXbkLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_del'] == 0) {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'ColseLiveRoom\''.',6)">禁用</a> | ';
            } else {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'OpenLiveRoom\''.',6)">启用</a> | ';
            }
            ...
            if ($val['is_del'] == 1){
                $val['DOACTION'] .= ' | <a title="此操作会彻底删除数据" href="javascript:void(0)"  onclick="admin.livedoaction('.$val['id'].',\'DelLiveRoom\''.',6)">彻底关闭</a>  ';
            }
    ...
    private function _getEeoXbkLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_del'] == 0) {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'ColseLiveRoom\''.',7)">禁用</a> | ';
            } else {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'OpenLiveRoom\''.',7)">启用</a> | ';
            }
            ...
            if ($val['is_del'] == 1){
                $val['DOACTION'] .= ' | <a title="此操作会彻底删除数据" href="javascript:void(0)"  onclick="admin.livedoaction('.$val['id'].',\'DelLiveRoom\''.',7)">彻底关闭</a>  ';
            }
    ...
    private function _getTkLiveList($type,$limit,$order,$map) {
        ...
        foreach($list['data'] as &$val){
            ...
            if ($val['is_active'] == 1) {
                ...
                $val['is_active'] = '<span title="审核此直播间" href="javascript:;" onclick="admin.livedoaction('.$val['id'].',\'ActiveLiveRoom\''.',4)" style="color: red;">未审核</span> ';
            }
            ...
            if ($val['is_del'] == 0) {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'ColseLiveRoom\''.',8)">禁用</a> | ';
            } else {
                $val['DOACTION'] .= '<a href="javascript:void(0)" onclick="admin.livedoaction('.$val['id'].',\'OpenLiveRoom\''.',8)">启用</a> | ';
            }
            ...
            if ($val['is_del'] == 1){
                $val['DOACTION'] .= ' | <a title="此操作会彻底删除数据" href="javascript:void(0)"  onclick="admin.livedoaction('.$val['id'].',\'DelLiveRoom\''.',8)">彻底关闭</a>  ';
            }
